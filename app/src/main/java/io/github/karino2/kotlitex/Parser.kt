        ...
        */
        ...
        init {
            ...
            for(ch in letters) {
                ...
                defineSymbol(Mode.MATH, Font.MAIN, NonAtoms.mathord, chstr, chstr)
                defineSymbol(Mode.TEXT, Font.MAIN, NonAtoms.textord, chstr, chstr)
            ...
            for (ch in mathTextSymbols) {
                val chstr = ch.toString()
                defineSymbol(Mode.MATH, Font.MAIN, NonAtoms.textord, chstr, chstr);
            }
    ...
    private fun parseSymbol(): ParseNode? {
        ...
        if (match != null) {
            for (i in 0..match.value.length){
                ...
                val command = unicodeAccents[accent]?.get(mode) ?: throw ParseError(
